...
- `model_file`
  ...
  The HDF5 file for the variant. If the file is not in the HDF5 format, it is likely that things will break. It saves the variant to the directory set in [`MODEL_FOLDER`](../settings#model_folder) using a [custom storage class]() and changes the name of the uploaded file using the [`get_model_filename()`](#get_model_filenamefilename) method.
...
- `clr`
  ...
  This is the usual report generated by `classification_report()`, transformed into a format which makes it easier to create an HTML table. This is done through the [`clr_to_tabular()`]() utility.
